...
const TableDemo = () => {
    ...
    const [customers2, setCustomers2] = useState<any>(null);
    const [customers3, setCustomers3] = useState<any>(null);
    const [filters1, setFilters1] = useState<any>('');
    const [loading1, setLoading1] = useState<boolean>(true);
    const [loading2, setLoading2] = useState<boolean>(true);
    const [products, setProducts] = useState<any>();
    const [idFrozen, setIdFrozen] = useState<any>(false);
    ...
    const representativeBodyTemplate = (rowData: any) => {
        ...
        return (
            <React.Fragment>
                <img alt={representative.name} src={`assets/demo/images/avatar/${representative.image}`} onError={(e: any) => e.target.src = 'https://www.primefaces.org/wp-content/uploads/2020/05/placeholder.png'} width={32} style={{ verticalAlign: 'middle' }} />
    ...
    const statusFilterTemplate = (options: any) => {
        return <Dropdown value={options.value} options={statuses} onChange={(e: any) => options.filterCallback(e.value, options.index)} itemTemplate={statusItemTemplate} placeholder="Select a Status" className="p-column-filter" showClear />;
    }
    ...
    const verifiedFilterTemplate = (options: any) => {
        return <TriStateCheckbox value={options.value} onChange={(e: any) => options.filterCallback(e.value)} />
    }

    const expandAll = () => {
        let _expandedRows: any = {};
        products.forEach((p: any) => _expandedRows[`${p.id}`] = true);
    ...
    const imageBodyTemplate = (rowData: any) => {
        return <img src={`assets/demo/images/product/${rowData.image}`} onError={(e: any) => e.target.src = 'https://www.primefaces.org/wp-content/uploads/2020/05/placeholder.png'} alt={rowData.image} className="shadow-2" width={100} />;
    }
